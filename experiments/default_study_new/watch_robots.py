...
class Simulator:
    ...
    async def simulate(self) -> None:
        ...
        self.experiments_name = ['tilted5']
        self.runs = [4]#list(range(1, 20+1))
        self.generations = [20]
        self.bests = 1
    ...
    async def recover(self, db, gen, path):
        async with AsyncSession(db) as session:
            ...
            for idx, r in enumerate(rows[0:self.bests]):
                ...
                env = Environment()
                x_rotation_degrees = float(env_conditions[1][2])
                ...
                runner = LocalRunner(LocalRunner.SimParams(), headless=True, env_conditions=env_conditions[1])
