...
# V2: maternal injection countering bias for tiny heads
...
class GRN:
    ...
    def __init__(self, max_modules, tfs, genotype, querying_seed, env_condition, n_env_conditions, plastic_body):
        ...
        if tfs == 'b':  # b=baseline, number of regulatory tfs equals number of modules tfs
            self.regulatory_tfs = 2
            self.structural_trs = two_modules
        elif tfs == 'r':  # r=more regulatory, number of regulatory tfs is double the number of modules tfs
            self.regulatory_tfs = 4
            self.structural_trs = two_modules

        elif tfs == 'm':  # m=more modules, number of modules tfs is double the number of regulatory tfs
            self.regulatory_tfs = 2
    ...
    def place_module(self, cell):
        ...
        concentration1 = sum(cell.transcription_factors[self.product_tfs[0]]) \
            if cell.transcription_factors.get(self.product_tfs[0]) else 0  # B - brick

        concentration2 = sum(cell.transcription_factors[self.product_tfs[1]]) \
            if cell.transcription_factors.get(self.product_tfs[1]) else 0  # A - joint

        concentration3 = sum(cell.transcription_factors[self.product_tfs[2]]) \
            if cell.transcription_factors.get(self.product_tfs[2]) else 0  # rotation
    ...
    def maternal_injection(self):
       ...
       # print(self.promotors)
        for ipromotor, promotor in enumerate(self.promotors):
          ...
          #  print(ipromotor,promotor[self.transcription_factor_idx])
            if promotor[self.transcription_factor_idx] in ["TF1", "TF2"]:
                first_tf_idx = ipromotor
              #  print('has!',first_tf_idx)
                break

        mother_tf_label = self.promotors[first_tf_idx][self.transcription_factor_idx]
       # print( mother_tf_label,float(self.promotors[first_tf_idx][self.transcription_factor_amount_idx]))
        mother_tf_injection = float(self.promotors[first_tf_idx][self.transcription_factor_amount_idx])
#
        first_cell = Cell()
